...
Original file is located at
    https://colab.research.google.com/drive/1NlVKiCJJ23fMyNFfKoE6j0r-eraydOXr
"""

import pandas as pd
import numpy as np

df = pd.read_csv('news_data_enc.csv', encoding="cp949")
df

df = df.drop('index', axis=1)
df

df['article']= df['article'].str.replace(pat=r'[^\w]',repl=r' ',regex=True)
df
...
for i in range(400):
  df["article"][i] = ' '.join(df["article"][i])

df

for i in range(400):
  df["title"][i] = ' '.join(df["title"][i])

df

df.to_csv('data(4).csv', encoding='utf-8-sig')